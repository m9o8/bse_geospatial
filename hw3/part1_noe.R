...
# Import SPEI index 
spei_index <- rast("hw3/data/spei01.nc")
...
ggplot(spei_regional, aes(x = year, y = mean_spei_region, color = region, group = region)) +
  ...
  geom_smooth(data = spei_regional, aes(x = year, y = mean_spei_region), 
              method = "loess", color = "black", se = TRUE, linewidth = 1.2, inherit.aes = FALSE) +
